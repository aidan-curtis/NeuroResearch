    ...
    for window = 1:NUMBER_OF_WINDOWS
        ...
        for i = indecies_case_a
           for j = indecies_case_b
               current = current + 1;
               groups(1, current) = abs(one_dim_feature_vector_array(i, channel, window)- one_dim_feature_vector_array(j, channel));
           end
        end
        ...
        t_val(window) = mean_val/(standard_deviation/sqrt(groups_size));
        
    end
    t_vals(channel) = mean(t_val);
end
